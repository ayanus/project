...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        while ($recipe = $result_recipe->fetch_assoc()) {
            ...
            while ($total_quantity_needed > 0 && $bee_stock = $result_bee_stock->fetch_assoc()) {
                ...
                if ($bee_stock_quantity <= $total_quantity_needed) {
                    ...
                    $stmt_update_bee_stock->bind_param("i", $b_keep_dt_id);
        ...
        // อัปเดตจำนวนสินค้าเพิ่มขึ้นในตารางสินค้า
        $sql_update_product_stock = "UPDATE products SET quantity = quantity + ? WHERE product_id = ?";
        $stmt_update_product_stock = $conn->prepare($sql_update_product_stock);
        $stmt_update_product_stock->bind_param("ii", $quantity_units, $product_id);
